...
def main():
    ...
    for row in df.itertuples(index=False):
        ...
        for i in range(3):
            # First test
            filename_test1 = f"output/qubo_test/test6_{n}_{m}_shots_run{i}.txt"
            if os.path.isfile(filename_test1):
                with open(filename_test1, "r") as f:
                    ...
                    #     print(match_qubo_size)
                    #     # if match_qubo_size:
                    #     #     qubo_size.append(int(match.group(1)))
                    #     #     break
                    # # Find all energies and if the solutions are correct
                    text = f.read()
                    time.append([float(x) for x in re.findall(r"Time to compute the solution:\s*(\d+\.\d+)\s*s", text)])
